...
public final class NamelessAPI {
	...
	public NamelessException checkWebAPIConnection() {		
		try {
			final Request request = new Request(this.apiUrl, this.userAgent,  Request.Action.INFO);
			request.connect();
	...
	public List<Announcement> getAnnouncements() throws NamelessException {
		final Request request = new Request(this.apiUrl, this.userAgent,  Request.Action.GET_ANNOUNCEMENTS);
		request.connect();
	...
	public List<Announcement> getAnnouncements(final UUID uuid) throws NamelessException {
		final Request request = new Request(this.apiUrl, this.userAgent, Request.Action.GET_ANNOUNCEMENTS, new ParameterBuilder().add("uuid", uuid).build());
		request.connect();
	...
	public void submitServerInfo(final String jsonData) throws NamelessException {
		final Request request = new Request(this.apiUrl, this.userAgent, Request.Action.SERVER_INFO, new ParameterBuilder().add("info", jsonData).build());
		request.connect();
		if (request.hasError()) throw new ApiError(request.getError());
	}
	...
	public Website getWebsite() throws NamelessException {
		...
		if (updateAvailable) {
			...
			update = new Website.Update(isUrgent, updateVersion);
		} else {
			update = null;
		}
	...
	public boolean validateUser(final UUID uuid, final String code) throws NamelessException {
		...
		final Request request = new Request(this.apiUrl, this.userAgent, Request.Action.VALIDATE_USER, parameters);
		request.connect();
		if (request.hasError()) {
	...
	public Map<UUID, String> getRegisteredUsers(final boolean hideInactive, final boolean hideBanned) throws NamelessException {
		final Request request = new Request(this.apiUrl, this.userAgent, Request.Action.LIST_USERS);
		request.connect();
		if (request.hasError())
